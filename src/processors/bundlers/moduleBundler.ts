 ...
 * </p>
 *
 * mode The embedding mode. Either 'provided', 'raw', 'preload', 'require' or 'bundleInfo'
 *
 * filters List of modules declared as glob patterns (resource name patterns) that should be
 * in- or excluded.
 ...
 * };
 *
 * [resolve=false] Whether (transitive) dependencies of modules that match the given filters
 *		should be resolved and added to the module set
 * [resolveConditional=false] Whether conditional dependencies of modules should be resolved
 * 		and added to the module set for this section
 * [renderer=false] Whether renderers for controls should be added to the module set
 * [declareRawModules=false] Whether raw modules should be declared after jQuery.sap.global
 *		became available. With the usage of the ui5loader, this flag should be set to 'false'
 * [sort=true] Whether the modules should be sorted by their dependencies
 * [async=true] Whether the `require` section of the module should be loaded asynchronously.
 ...
 *
 * [defaultFileTypes=[".js", ".control.xml", ".fragment.html", ".fragment.json", ".fragment.xml", ".view.html", ".view.json", ".view.xml"]]
 * List of default file types to be included in the bundle
 *
 * sections List of module bundle definition sections.
 */
 ...
 *
 * [optimize=true] Whether the module bundle gets minified
 *
 * [sourceMap=true] Whether to generate a source map file for the bundle
 *
 * [decorateBootstrapModule=false] If set to 'false', bootable bundles won't be decorated
 * with an optimization marker
 *
 * [addTryCatchRestartWrapper=false] Whether to wrap bootable bundles with
 * a try/catch to filter out "Restart" errors
 *
 * [numberOfParts=1] The number of parts the module bundle should be splitted
 *
 * [ignoreMissingModules=false] When searching for modules which are optional for further
 ...
 * @param root0.options.moduleNameMapping
 * @param root0.options.targetUi5CoreVersion
 * @param root0.options.allowStringBundling
 */
