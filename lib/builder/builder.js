...
module.exports = {
	 ...
	 * @param {boolean} [parameters.clearDestPath=false] Decides whether project should clear the target path before build
	 ...
	 */
	build({
		tree, destPath, clearDestPath = false,
			...
			return depPromise.then(() => {
				...
				return projectType.build({
					...
					return new Promise((resolve) => {
						return (clearDestPath) ? rimraf(destPath, resolve) : resolve();
					});
				}).then(() => {
