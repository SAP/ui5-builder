...
export class Parallelizer {
	#log = getLogger("builder:utils:Parallelizer");
	#pool;
	...
	#getPool() {
		if (!this.#pool) {
			...
			const workerPath = fileURLToPath(
				new URL("./ParallelizerThread.js", import.meta.url)
			);
	...
	constructor() {
		if (!Parallelizer.#ensureSingleton) {
			throw new Error(
	...
	static getInstance() {
		if (!Parallelizer.#instance) {
			Parallelizer.#ensureSingleton = true;
			Parallelizer.#instance = new Parallelizer();
			Parallelizer.#ensureSingleton = false;
		}

		return Parallelizer.#instance;
	}

	async execInThread(args) {
		return this.#getPool().exec("execInThread", [args]);
	}
