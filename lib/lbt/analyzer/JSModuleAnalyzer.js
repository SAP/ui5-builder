...
class JSModuleAnalyzer {
	...
	analyze(ast, defaultName, info) {
		...
		function visit(node, conditional, parentNode) {
			...
			if ( Array.isArray(node) ) {
				node.forEach((child) => visit(child, conditional, nodeToAdd));
				return;
			}
			...
			case Syntax.CallExpression:
				if ( !conditional && isMethodCall(node, CALL_JQUERY_SAP_DECLARE) ) {
					...
					if ( iArg < args.length && isCallableExpression(args[iArg]) ) {
						...
						visit(args[iArg].body, conditional, nodeToAdd);
					}
					...
					if (info.format || calculateDepths(parentNode) < 5) {
					...
					visit(node.arguments, conditional, nodeToAdd);
					// NODE-TODO defaults of callee?
					visit(node.callee.body, conditional, nodeToAdd);
				} else {
					// default visit
					for ( const key of condKeys ) {
						visit(node[key.key], key.conditional || conditional, nodeToAdd);
					}
				}
				break;
			...
			case Syntax.IfStatement:
				...
				if ( node.test.type == Syntax.UnaryExpression &&
					...
					visit(node.consequent, conditional, nodeToAdd);
					visit(node.alternate, true, nodeToAdd);
				} else {
					// default visit
					for ( const key of condKeys ) {
						visit(node[key.key], key.conditional || conditional, nodeToAdd);
		...
		function calculateDepths(nodeWrapper) {
			let count = 0;
			let parent = nodeWrapper.parent;
			while (parent) {
				count++;
		...
		function onSapUiRequireSync(node, conditional, name) {
			const args = node.arguments;
			const nArgs = args.length;
			const i = 0;
			...
			if ( i < nArgs ) {
				if ( isString(args[i]) ) {
					let moduleName;
					if (name == null) {
						moduleName = ModuleName.fromRequireJSName( args[i].value );
					} else {
						moduleName = ModuleName.resolveRelativeRequireJSName(name, args[i].value);
					}
