...
const BindingParser = require("./BindingParser");
			...
			if ( FRAGMENTDEFINITION_MODULE !== moduleName ) {
				this.promises.push(
					...
					this._pool.findResource(moduleName).then( () => {
						return BindingParser.getBindingParser().then((oBindingParser) => {
							this._addDependency(moduleName);

							const title = getAttribute(node, "title");
							if (title) {
								const binding = oBindingParser.complexParser(title);
								log.warn(binding);
							}
							...
							// - ComponentContainer reference another component by 'name'
