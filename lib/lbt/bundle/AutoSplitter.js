		...
		if ( resource != null ) {
			...
			if ( /\.js$/.test(module) ) {
				...
				*/
				...
				// input encoding is read from project configuration, default is "ISO-8859-1"
				// since AutoSplitter is also used when serving non-project resources (e.g. dependencies) *.properties files should be escaped
				const propertiesFileEncoding = resource.getProject() && resource.getProject().resources.propertiesFileEncoding;
				const encoding = nonAsciiEscaper.getEncodingFromNiceName(propertiesFileEncoding || "ISO-8859-1");
				...
				return fileContent.toString().length;
