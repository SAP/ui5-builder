				...
				if (mapping.source) {
					this._sourceMap.addMapping({
						...
						source: mapping.originalLine != null ?
							path.join(path.relative(path.dirname(this._sourceMap._file), path.dirname(resource.name)), this.optimize ? ModuleName.getDebugName(path.basename(resource.name)) : path.basename(resource.name)) : null,
						name: mapping.name
					});
				}
			});
			...
			for ( const module of sequence ) {
				if ( /\.js$/.test(module) ) {
					...
					if ( ast ) {
						...
						const {compressedContent, contentMap} = {...await this.compressJS(astAsCode.code, resource, astAsCode.map.toString())};
