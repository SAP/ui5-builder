...
class BundleBuilder {
	...
	async writePreloadModule(module, info, resource, avoidLazyParsing) {
		...
		if ( /\.js$/.test(module) && (info == null || !info.requiresTopLevelScope) ) {
			...
			// same as for other text files, input encoding is read from project configuration, default is ISO-8859-1
			// since Builder is also used when building non-project resources (e.g. dependencies)
			// *.properties files should be escaped if encoding option is specified
			const propertiesFileEncoding = resource.getProject()
				&& resource.getProject().resources
				&& resource.getProject().resources.configuration
				&& resource.getProject().resources.configuration.propertiesFileEncoding;
			if (propertiesFileEncoding) {
				const encoding = nonAsciiEscaper.getEncodingFromAlias(propertiesFileEncoding);
			...
			outW.write( makeStringLiteral( fileContent.toString() ) );
		} else {
