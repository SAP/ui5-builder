		...
		if ( this.options.usePredefineCalls ) {
			...
			for ( const module of sequence ) {
				if ( /\.js$/.test(module) ) {
					...
					let resourceSourceMap = hasInlineSourcemap ?
						Buffer.from(inlineSourcemap[1], "base64").toString() : sourceMapContent;
					...
					if (resourceSourceMap != null) {
						...
						resourceSourceMap.sources[0] = path.posix.join(
							path.posix.dirname(resourceName), resourceSourceMap.sources[0]
						);
					}
					...
					if ( ast ) {
						...
						let preloadContent = "";
						if (this.optimize) {
							const {compressedContent, contentMap} = {
								...this.compressJSAST(result.ast, resource, resourceSourceMap)
							};
