...
class BundleBuilder {
	...
	async writePreloadFunction(section) {
		...
		// TODO: Drop this configuration. It's undocumented and there's no real benefit at runtime
	...
	async compressJS(fileContent, resource) {
		if ( this.optimize ) {
			...
			try {
				...
				console.log("error for " + resource.name);
			...
			for ( const module of sequence ) {
				if ( /\.js$/.test(module) ) {
					...
					if ( ast ) {
						outW.startSegment(module);
						outW.ensureNewLine();
						let astAsCode = escodegen.generate(ast, {comment: true});
						// Hacky workaround for https://github.com/estools/escodegen/issues/258
						//	Remove line break inserted between the end of a multi line comment and
						//	opening curly brackets (might be required for other characters as well, so really hacky)
						astAsCode = astAsCode.replace(/\*\/\n\s*{/g, "*/ {\n");
...
function rewriteDefine(targetBundleFormat, code, moduleName, avoidLazyParsing) {
	...
	let ast;
	try {
		ast = esprima.parseScript(code.toString(), {loc: true, comment: true, attachComment: true});
	} catch (e) {
	...
	if ( ast.type === Syntax.Program &&
		...
		// TODO: The following code is obsolete and can be removed:
