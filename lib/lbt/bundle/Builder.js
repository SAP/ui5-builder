...
class BundleBuilder {
	...
	async writeRawModule(moduleName, resource) {
		...
		if (this.options.sourceMap) {
			let moduleSourceMap;
			({moduleContent, moduleSourceMap} = await this.getSourceMapForModule(moduleName, moduleContent));
				...
				if ( /\.js$/.test(moduleName) ) {
					...
					if (this.options.sourceMap) {
						({moduleContent, moduleSourceMap} =
							await this.getSourceMapForModule(moduleName, moduleContent));
					}
	...
	async getSourceMapForModule(moduleName, moduleContent) {
		...
		if (sourceMapUrlMatch) {
			...
			const sourceMapPath = moduleName + ".map";
