...
class BundleBuilder {
	...
	async writeRaw(section) {
		...
		for ( const moduleName of section.modules ) {
			...
			if ( resource != null ) {
				...
				log.verbose("    %s (%d,%d)", moduleName, resource.info != null ? resource.info.size : -1, compressedSize);
	...
	async writeRawModule(moduleName, resource) {
		...
		if (this.options.sourceMap) {
			let moduleSourceMap;
			({moduleContent, moduleSourceMap} = await this.retrieveSourceMapForModule(moduleName, moduleContent));
				...
				if ( /\.js$/.test(moduleName) ) {
					...
					if (this.options.sourceMap) {
						({moduleContent, moduleSourceMap} =
							await this.retrieveSourceMapForModule(moduleName, moduleContent));
					}
...
async function rewriteDefine(targetBundleFormat, moduleContent, moduleName, moduleSourceMap) {
	let ast;
	try {
