...
const EVOBundleFormat = {
	...
	requireSequentially(modules) {
		if (modules.length === 0) {
			return "";
		}
		let code = "";
		for (let i = modules.length - 1; i >= 0; i--) {
			code = `sap.ui.require(["${toRequireJSName(modules[i])}"], () => {\n${code}\n});`;
		}
		return code;
	},
...
class BundleBuilder {
	...
	writeRequires(section) {
		...
		if (section.modules.length > 0) {
			this.writeWithSourceMap(this.targetBundleFormat.requireSequentially(section.modules));
		}
	}
