...
class BundleBuilder {
	...
	writeWithSourceMap(content) {
		const transientSourceName = `${this._bundleName}?inline-${this._inlineContentCounter++}`;
		const sourceMap = this.createTransientSourceMap({
			moduleName: transientSourceName,
			moduleContent: content,
			includeContent: true
		});
		this.addSourceMap(transientSourceName, sourceMap);
	...
	async writePreloadFunction(section) {
		...
		if ( sequence.length > 0 ) {
			...
			this.writeWithSourceMap(this.targetBundleFormat.afterPreloads(section));
		}
	...
	async getSourceMapForModule(moduleName, moduleContent, resource) {
		...
		if (!moduleSourceMap) {
			try {
				const sourceMapResource = await this.pool.findResource(resource.getPath() + ".map");
		...
		if (moduleSourceMap) {
			...
			moduleSourceMap = this.createTransientSourceMap({
				moduleName: path.posix.basename(resource.getPath()),
				moduleContent
			});
		}
	...
	createTransientSourceMap({moduleName, moduleContent, includeContent = false}) {
		const sourceMap = {
			version: 3,
			sources: [moduleName],
