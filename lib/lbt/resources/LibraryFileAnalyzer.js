...
let parser;
function parseString(content, cb) {
	if (!parser) {
	...
	return parser.parseString(content, cb);
}
...
function getDependencyInfos( name, content ) {
	const infos = {};
	// Fast check whether XML needs to be parsed at all
	if (content && content.includes("raw-module")) {
