...
class ResourcePool {
	constructor({ignoreMissingModules, ignoreDebugFiles} = {}) {
		this._ignoreMissingModules = !!ignoreMissingModules;
		this._ignoreDebugFiles = !!ignoreDebugFiles;
		// this._roots = [];
		this._resources = [];
	...
	*/
	...
	addResource( resource ) {
		...
		const debugName = ModuleName.getDebugName(resource.name);
		if ( debugName ) {
			const debugResource = this._resourcesByName.get(debugName);
			if ( debugResource ) {
				// Make non-debug resource aware of its debug-resource
				resource.setDebugResource(debugResource.resource);

				if (this._ignoreDebugFiles) {
					const debugResourceIdx = this._resources.indexOf(debugResource);
					if ( debugResourceIdx !== -1 ) {
						console.log("Removing debug resource " + debugResource.name);
						// Remove debug-resource from list of resources
						this._resources.splice(debugResourceIdx, 1);
						this._resourcesByName.delete(debugResource);
					}
				}
			}
		} else {
			const nonDebugName = ModuleName.getNonDebugName(resource.name);
			const nonDebugResource = this._resourcesByName.get(nonDebugName);
			if (nonDebugResource) {
				if (nonDebugResource) {
					// Make non-debug resource aware of its debug resource
					nonDebugResource.setDebugResource(resource.resource);
				}
				if (this._ignoreDebugFiles) {
					console.log("Ignoring debug resource " + resource.name);
					return;
				}
			}
		}
