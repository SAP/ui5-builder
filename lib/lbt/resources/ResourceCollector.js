...
class ResourceCollector {
	...
	async enrichWithDependencyInfo(resourceInfo) {
		return this._pool.getModuleInfo(resourceInfo.name).then(async (moduleInfo) => {
			...
			this._applyDependencies(resourceInfo, moduleInfo);
			...
			if ( moduleInfo.subModules.length > 0 ) {
				...
				await Promise.all(moduleInfo.subModules.map(async (subModule) => {
					...
					try {
						...
						// TODO: Decide on message and log level
						log.warn("Missing module? " + subModule);
					}
					if (subModuleInfo) {
						// Inherit dependencies
						this._applyDependencies(resourceInfo, subModuleInfo);
	...
	_applyDependencies(resourceInfo, moduleInfo) {
