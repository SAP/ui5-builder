...
class ResourceCollector {
	...
	async enrichWithDependencyInfo(resourceInfo) {
		return this._pool.getModuleInfo(resourceInfo.name).then(async (moduleInfo) => {
			...
			this._applyModuleInfoDependenciesToResourceInfo(resourceInfo, moduleInfo);
			...
			if ( moduleInfo.subModules.length > 0 ) {
				...
				await Promise.all(moduleInfo.subModules.map(async (subModule) => {
					// Add subModule as included
					resourceInfo.included.add(subModule);
					...
					if (subModuleInfo) {
						...
						this._applyModuleInfoDependenciesToResourceInfo(resourceInfo, subModuleInfo);
	...
	_applyModuleInfoDependenciesToResourceInfo(resourceInfo, moduleInfo) {
