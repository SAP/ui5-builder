...
class ResourcesList {
	...
	add(info, shareDebugInformation=true) {
		...
		if ( myInfo == null && shareDebugInformation) {
			...
			const nonDbgName = ResourcesList.getNonDebugName(relativeName);
			const dbgName = ResourcesList.getDebugName(relativeName);
			if ( nonDbgName != null && this.resourcesByName.has(nonDbgName) ) {
				...
				if (source.i18nName) {
					myInfo.i18nName = ResourcesList.makePathRelativeTo(this.name, source.i18nName);
				}
				...
				myInfo.module = ResourcesList.getNonDebugName(source.module);
				this.resources.push(myInfo);
				this.resourcesByName.set(relativeName, myInfo);
			}
		}
		...
		if ( myInfo == null ) {
			...
			myInfo.module = ResourcesList.getNonDebugName(info.name);
			this.resources.push(myInfo);
			this.resourcesByName.set(relativeName, myInfo);
		}
		myInfo.copyFrom(this.name, info);
		if (info.i18nName) {
			myInfo.i18nName = ResourcesList.makePathRelativeTo(this.name, info.i18nName);
		}
	}
