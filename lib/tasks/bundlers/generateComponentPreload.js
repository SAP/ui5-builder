...
module.exports = function({
	...
	return combo.byGlob("/resources/**/*.{js,json,xml,html,properties,library}")
		...
		.then((processedResources) => {
			return Promise.all(processedResources.map(async ([componentPreload, componentPreloadMap]) => {
				if (taskUtil) {
					taskUtil.setTag(componentPreload, taskUtil.STANDARD_TAGS.IsBundle);
				}
				await workspace.write(componentPreload);
				await workspace.write(componentPreloadMap);
			}));
		});
};
