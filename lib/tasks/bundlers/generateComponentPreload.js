...
module.exports = function({
	...
	return combo.byGlob("/resources/**/*.{js,json,xml,html,properties,library}")
		.then(async (resources) => {
			...
			// TODO: check for not applied "allFilterExcludes" and log warning/error (or throw error?)
			...
			return Promise.all(allNamespaces.map((namespace) => {
				...
				//
				// sap/app1/localService/**
				// !sap/app1/localService/Hundekuchen.js
				// !sap/
				//

				// Add configured excludes for namespace at the end of filter list
				filterExcludes.forEach((filterExclude) => {
