...
module.exports = function({workspace, dependencies, options}) {
	...
	return combo.byGlob("/**/*.{js,json,xml,html,properties,library}").then((resources) => {
		...
		if (options.projectName === "sap.ui.core") {
			...
			function getSapUiCoreBunDef(bundleName, preload=false) {
			...
			function getNojQueryBunDef(bundleName, preload=false) {
				const bundleDefinition = {
					name: bundleName,
					sections: []
				};
			...
			p = Promise.all([
				moduleBundler({
					options: {
						...
						bundleDefinition: getSapUiCoreBunDef("sap-ui-core.js", true)
					},
					resources
				}),
				moduleBundler({
					options: {
						...
						bundleDefinition: getSapUiCoreBunDef("sap-ui-core-dbg.js")
					},
					resources
				}),
				moduleBundler({
					options: {
						...
						bundleDefinition: getNojQueryBunDef("sap-ui-core-nojQuery.js", true)
					},
					resources
				}),
				moduleBundler({
					options: {
						...
						bundleDefinition: getNojQueryBunDef("sap-ui-core-nojQuery-dbg.js")
					},
					resources
				}),
			]).then((results) => {
