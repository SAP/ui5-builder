...
export default async function({workspace, taskUtil, options = {}}) {
	...
	function updateFlexLibDependency(data) {
	...
	function updateFlexBundleProp(data, hasFlexibilityBundle = false) {
		data["sap.ui5"] ??= {};
		// Set flexBundle property based on flexibility-bundle.json existence
		data["sap.ui5"].flexBundle = hasFlexibilityBundle;
		log.verbose(`Setting flexBundle to '${hasFlexibilityBundle}' based on ` +
			`${hasFlexibilityBundle ? "presence" : "absence"} of 'flexibility-bundle.json'`);
	}

	async function updateFLdependency(hasFlexibilityBundle = false, updateFlexLib = true) {
		...
		if (updateFlexLib) {
			updateFlexLibDependency(manifestContent);
		}
		updateFlexBundleProp(manifestContent, hasFlexibilityBundle);
	...
	// Check if flexibility-bundle.json exists
	const flexibilityBundleResource = await workspace.byPath(`${pathPrefix}/flexibility-bundle.json`);
	const hasFlexibilityBundle = flexibilityBundleResource !== null;

	let flexBundlePropUpdated = false;
	...
	if (allResources.length > 0) {
		...
		if (processedResources.length > 0) {
			await updateFLdependency(hasFlexibilityBundle);
			flexBundlePropUpdated = true;
		}
	...
	// Always update the manifest to set the flexBundle property if not already updated
	// This ensures the property is set regardless of whether changes exist
	if (!flexBundlePropUpdated) {
		await updateFLdependency(hasFlexibilityBundle, false);
	}
}
