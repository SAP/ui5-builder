...
export default async function({
	workspace, invalidatedResources, taskUtil,
	options: {pattern, omitSourceMapResources = false, useInputSourceMaps = true}
}) {
	let resources;
	if (invalidatedResources) {
		resources = await Promise.all(invalidatedResources.map((resource) => workspace.byPath(resource)));
	} else {
	...
	return Promise.all(processedResources.map(async ({
		...
		return Promise.all([
			workspace.write(resource, undefined, resource),
			workspace.write(dbgResource, undefined, resource),
			workspace.write(sourceMapResource, undefined, resource),
			dbgSourceMapResource && workspace.write(dbgSourceMapResource, undefined, resource)
		]);
	}));
}
