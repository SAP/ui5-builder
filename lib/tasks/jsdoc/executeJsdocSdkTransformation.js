...
const executeJsdocSdkTransformation = async function({workspace, dependencies, options} = {}) {
	if (!options || !options.projectName || !options.librariesPattern) {
	...
	const [apiJsons, dotLibraries, depApiJsons] = await Promise.all([
		...
		workspace.byGlob(options.librariesPattern),
	...
	if (!apiJsons.length) {
		...
		log.warn(`Found more than one api.json resources for project ${options.projectName}. ` +
			`Will use the first resource, found at ${apiJsons[0].getPath()}`);
	}
	if (!dotLibraries.length) {
		...
		log.warn(`Found more than one .library resources for project ${options.projectName}. ` +
			`Will use the first resource, found at ${dotLibraries[0].getPath()}`);
	}
