...
async function generateThemeDotTheming({workspace, combo, themeFolder}) {
	...
	let libraryNamespace;
	let libraryName;
	if (libraryMatch) {
		libraryNamespace = libraryMatch[1];
		libraryName = libraryNamespace.replace(/\//g, ".");
	} else {
	...
	if (coreDotThemingResource) {
		...
		// Check if the theme referenced as "oExtends" in exists for the current library
		// and if not, use "base" instead.
		const newDotThemingContent = JSON.parse(await newDotThemingResource.getString());
		if (typeof newDotThemingContent.oExtends === "string") {
			const themeDir = await combo.byPath(
				`/resources/${libraryNamespace}/themes/${newDotThemingContent.oExtends}/`, {
					nodir: false
				}
			);
			if (!themeDir) {
				newDotThemingContent.oExtends = "base";
				newDotThemingResource.setString(JSON.stringify(newDotThemingContent, null, 2));
			}
		}
	} else {
