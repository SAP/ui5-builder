...
const CSS_VARIABLES_SOURCE_LESS = "css_variables.source.less";
const CSS_VARIABLES_LESS = "css_variables.less";
...
async function createCssVariablesLessResource({workspace, dependencies, themeFolder}) {
	const pathToRoot = getPathToRoot(themeFolder);
	const combo = new ReaderCollectionPrioritized({
		name: `createCssVariablesLessResource - prioritize workspace over dependencies:`,
...
`/* NOTE: This file was generated as an optimized version of "${CSS_VARIABLES_SOURCE_LESS}" \
for the Theme Designer. */\n\n`;

	if (themeName !== "base") {
		const cssVariablesSourceLessResource = await workspace.byPath(
			posixPath.join(themeFolder, CSS_VARIABLES_SOURCE_LESS)
		);

		if (!cssVariablesSourceLessResource) {
			throw new Error(`Could not find file "${CSS_VARIABLES_SOURCE_LESS}" in theme "${themeFolder}"`);
		}

		const cssVariablesSourceLess = await cssVariablesSourceLessResource.getString();

		cssVariablesLess += lessImport(`../base/${CSS_VARIABLES_LESS}`);
		cssVariablesLess += `
/* START "${CSS_VARIABLES_SOURCE_LESS}" */
${cssVariablesSourceLess}
/* END "${CSS_VARIABLES_SOURCE_LESS}" */
...
`;
	...
	const cssVariablesLessResource = new Resource({
		path: posixPath.join(themeFolder, CSS_VARIABLES_LESS),
		string: cssVariablesLess
	});
	await workspace.write(cssVariablesLessResource);
}
...
module.exports = async function({workspace, dependencies, options: {projectName, version, namespace}}) {
	...
	if (hasCssVariables) {
		// css_variables.less
		const cssVariablesLessResource = cssVariablesSourceLessResource.map((cssVariableSourceLess) => {
			const themeFolder = posixPath.dirname(cssVariableSourceLess.getPath());
			log.verbose(`Generatingc css_variables.less for theme ${themeFolder}`);
			return createCssVariablesLessResource({
				workspace, dependencies, themeFolder
			});
		});
		await Promise.all(
			cssVariablesLessResource.map(async (resource) => {
				if (resource) {
					await workspace.write(resource);
				}
			})
