...
import {uint8ToResources, makeTransferableResource} from "../processors/themeBuilderWorker.js";
...
export default async function({
	...
	if (useWorkers) {
		...
		processedResources = await Promise.all(themeResources.map(async (themeRes) => await buildThemeInWorker({
		...
		const buildTheme = (await import("../processors/themeBuilder.js")).default;

		processedResources = await buildTheme({
	...
	return Promise.all(processedResources.map((res) => workspace.write(res)));
}
