...
import {
	...
	serializeResources,
	DuplexCollectionMainInterface,
	PoolDispatcher,
} from "../lbt/utils/PoolDispatcher.js";
...
async function buildThemeInWorker(taskUtil, options, transferList, {resources, workspace, dependencies}) {
	...
	const args = {
		options,
		resources: await serializeResources(resources),
		workspace: new DuplexCollectionMainInterface(workspace),
		dependencies: new DuplexCollectionMainInterface(dependencies),
	};
	const url = fileURLToPath(new URL("../processors/themeBuilderWorker.js", import.meta.url));

	const processor = PoolDispatcher.getInstance().getProcessor(url);
	return processor.execute("execThemeBuild", args);
}
...
export default async function({
	...
	if (useWorkers) {
		...
		processedResources = await Promise.all(themeResources.map(async (themeRes) => {
			...
			{resources: [themeRes], workspace, dependencies});
