...
import {uint8ToResources, resourcesToUint8, FsMainThreadInterface} from "../processors/themeBuilderWorker.js";
...
export default async function({
	...
	if (useWorkers) {
		...
		processedResources = await Promise.all(themeResources.map(async (themeRes) => {
			...
			// Those resources would be converted to transferable objects. After they are sent to the
			// worker thread, transferrableThemeRes and transferrableThemeResBuffer would be empty, but
			// the original resources would remain untouched. The benefit is that those objects are not carried/copied
			// over the communication channel.
			const transferrableThemeRes = await resourcesToUint8([themeRes]);
			const transferrableThemeResBuffer = transferrableThemeRes.map((res) => res.transferable.buffer);

			const result = await buildThemeInWorker(taskUtil, {
				fsInterfacePort: port2,
				themeResources: transferrableThemeRes,
				options: {
					compress,
					cssVariables: !!cssVariables,
				},
			}, [port2, ...transferrableThemeResBuffer]);

			threadMessageHandler.endCommunication(port1);

			return result;
		}))
			.then((resources) => resources.reduce((acc, res) => acc.concat(res), []))
			.then(uint8ToResources);
