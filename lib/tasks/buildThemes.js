...
export default async function({
	...
	if (useWorkers) {
		const modulePath = fileURLToPath(new URL("../processors/themeBuilder.js", import.meta.url));
		const processor = PoolDispatcher.getInstance().getProcessor(modulePath);
		processedResources = await Promise.all(themeResources.map(async (themeRes) => {
			return processor.execute("default", {
				resources: [themeRes],
