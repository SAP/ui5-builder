...
export default async function({
	...
	const pAllResources = workspace._byGlob(inputPattern);
	...
	if (librariesPattern) {
		...
		pAvailableLibraries = combo._byGlob(librariesPattern);
	}
	if (themesPattern) {
		// If a themesPattern is given
		//	we will use it to reduce the set of themes that will be built
		pAvailableThemes = combo._byGlob(themesPattern, {nodir: false});
	}
