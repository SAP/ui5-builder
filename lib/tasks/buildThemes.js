...
import {uint8ToResources, makeTransferableResource, FsMainThreadInterface} from "../processors/themeBuilderWorker.js";
...
export default async function({
	...
	if (useWorkers) {
		...
		// const allResourcesTransferable = await makeTransferableResource(allResources);
		// const channel = new MessageChannel();
		// const threadMessageHandler = new FsMainThreadInterface(channel.port1);
		// threadMessageHandler.handleResourceRequests(fsInterface(combo));


		processedResources = await Promise.all(themeResources.map(async (themeRes) => {
			// New channel for each request. Once the port2 is sent, it gets "lost" for the main thread.
			// So, for every new thread, a new channel should be opened.
			const {port1, port2} = new MessageChannel();
			const threadMessageHandler = new FsMainThreadInterface(port1);
			threadMessageHandler.handleResourceRequests(fsInterface(combo));
