...
import {uint8ToResources, resourcesToUint8} from "../processors/themeBuilderWorker.js";
...
export default async function({
	...
	if (useWorkers) {
		...
		const allResourcesTransferable = await resourcesToUint8(allResources);
		...
		processedResources = await Promise.all(themeResources.map(async (themeRes) => await buildThemeInWorker({
			...
			themeResources: await resourcesToUint8([themeRes]),
			options: {
				compress,
