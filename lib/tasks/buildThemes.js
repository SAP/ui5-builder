...
export default async function({
	...
	console.log(`\n${projectName}: Main thread: preprocessing-`, performance.now() - timer);

	const processedResources = await buildTheme({
		allResources: await makeTransferableResource(allResources),
		themeResources: await makeTransferableResource(themeResources),
		options: {
			projectName,
			compress,
	...
	console.log(`\n${projectName}: Main thread: after buildTheme-`, performance.now() - timer);

	await Promise.all(processedResources.map((rawResource) => {
		return workspace.write(createResource(rawResource));
	}));

	console.log(`\n${projectName}: Main thread: re-creation of resources before save-`, performance.now() - timer);
}
