...
class AbstractBuilder {
	...
	/**
	 * Appends the list of 'excludes' to the list of 'patterns'. To harmonize both lists, the 'excludes' are
	 * negated and a "/resources/" prefix is added to make them absolute.
	 *
	 * @private
	 ...
	 */
	enhancePatternWithExcludes(patterns, excludes = []) {
		excludes.forEach((exclude) => {
			if (exclude.startsWith("!")) {
				patterns.push(`/resources/${exclude.slice(1)}`);
			} else {
				patterns.push(`!/resources/${exclude}`);
			}
		});
	}
}
