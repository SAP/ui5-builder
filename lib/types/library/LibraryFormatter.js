...
class LibraryFormatter extends AbstractUi5Formatter {
	...
	validate() {
		...
		return Promise.resolve().then(() => {
			...
			if (!project.resources.propertiesFileEncoding) {
				project.resources.propertiesFileEncoding = "ISO-8859-1";
			}
			if (!["ISO-8859-1", "UTF-8"].includes(project.resources.propertiesFileEncoding)) {
				throw new Error(`Invalid properties file encoding specified for project ${project.id}: ` +
					`encoding provided: ${project.resources.propertiesFileEncoding}. Must be either "ISO-8859-1" or "UTF-8".`);
			}
