...
class LibraryFormatter extends AbstractUi5Formatter {
	...
	validate() {
		...
		return Promise.resolve().then(() => {
			...
			if (!project.resources.configuration.propertiesFileEncoding) {
				project.resources.configuration.propertiesFileEncoding = "ISO-8859-1";
			}
			if (!["ISO-8859-1", "UTF-8"].includes(project.resources.configuration.propertiesFileEncoding)) {
				throw new Error(`Invalid properties file encoding specified for project ${project.id}: ` +
					`encoding provided: ${project.resources.configuration.propertiesFileEncoding}. Must be either "ISO-8859-1" or "UTF-8".`);
			}
