...
class LibraryBuilder extends AbstractBuilder {
	addStandardTasks({resourceCollections, project, log, taskUtil}) {
		...
		function getJsProcessingPattern() {
			const {namespace} = project.metadata;
			const pattern = ["/resources/**/*.js"];

			// Library-specific excludes
			switch (namespace) {
			case "sap/ui/export":
			case "sap/ui/richtexteditor":
				pattern.push(`!/resources/${namespace}/js/**/*.js`);
				break;
			case "sap/makit":
				pattern.push(`!/resources/${namespace}/js/**/*.js`);
				pattern.push(`!/resources/${namespace}/localization/**/*.js`);
				break;
			case "sap/ndc":
				pattern.push(`!/resources/${namespace}/thirdparty/**/*.js`);
				break;
			case "sap/viz":
				pattern.push(`!/resources/${namespace}/resources/**/*.js`);
				pattern.push(`!/resources/${namespace}/ui5/container/libs/locale/**/*.js`);
				pattern.push(`!/resources/${namespace}/ui5/controls/libs/resources/locale/**/*.js`);
				pattern.push(`!/resources/${namespace}/ui5/controls/libs/sap-viz-vizframe/resources/locale/**/*.js`);
				break;
			}
			log.warn(pattern);
			return pattern;
		...
		this.addTask("createDebugFiles", async () => {
			return getTask("createDebugFiles").task({
				workspace: resourceCollections.workspace,
				options: {
					pattern: getJsProcessingPattern()
				}
			});
		});
