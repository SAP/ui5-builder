...
export default async function execMinification({
	...
	try {
		const minifyResult = await minify({
			...
			[dbgFilename]: Buffer.from(code.buffer, code.byteOffset, code.length).toString()
		}, {
			output: {
		...
		const codeBuffer = Buffer.from(minifyResult.code);
		const result = {
			code: new Int8Array(codeBuffer.buffer, codeBuffer.byteOffset, codeBuffer.length)
		};
		if (minifyResult.map) {
			const mapBuffer = Buffer.from(minifyResult.map);
			result.map = new Int8Array(mapBuffer.buffer, mapBuffer.byteOffset, mapBuffer.length);
		}
		return new workerpool.Transfer(result, [
			result.code.buffer,
			result.map?.buffer
		]);
	} catch (err) {
