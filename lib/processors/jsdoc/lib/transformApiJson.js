...
function transformer(sInputFile, sOutputFile, sLibraryFile, vDependencyAPIFiles, sFAQDir, options) {
	...
	let formatters = {
		...
		handleExternalUrl: function (sTarget, sText, bSAPHosted) {
			// Check if the external domain is SAP hosted
			bSAPHosted = bSAPHosted || /^https?:\/\/([\w.]*\.)?(?:sap|hana\.ondemand|sapfioritrial)\.com/.test(sTarget);
			...
			let oSelf = this._oTopicData,
				...
				oOptions = {
					linkFormatter: function (sTarget, sText) {
						...
						if (aMatch) {
							return this.handleExternalUrl("test-resources/" + aMatch[1], sText, true);
						}
