...
function analyzeModuleDefinition(node) {
	let definedModuleName;
	const args = node.arguments;
	let arg = 0;
	if ( arg < args.length
		 && args[arg].type === Syntax.Literal && typeof args[arg].value === 'string' ) {
		definedModuleName = args[arg].value;
		arg++;
	}
	if (definedModuleName) {
		warning(`module explicitly defined a module name '${definedModuleName}'`);
		currentModule.name = definedModuleName;
		const resourceModuleName = getModuleName(currentModule.resource);
		if (definedModuleName !== resourceModuleName) {
			warning(`module name differs from resource name: expected '${resourceModuleName}' but was ${definedModuleName}'`);
		}
	}
