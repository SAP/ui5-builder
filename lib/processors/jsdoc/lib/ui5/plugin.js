...
function createAutoDoc(oClassInfo, classComment, node, parser, filename, commentAlreadyProcessed) {
			...
			if ( !isEmpty(oClassInfo.properties)
				...
				if ( !isEmpty(oClassInfo.properties) ) {
					...
					for (n in oClassInfo.properties) {
						lines.push("<li>{@link " + rname("get", n) + " " + n + "} : " +
							oClassInfo.properties[n].type + (oClassInfo.properties[n].defaultValue !== null ?
							" (default: " + oClassInfo.properties[n].defaultValue + ")" : "") +
							(oClassInfo.defaultProperty == n ? " (default)" : "") + "</li>");
					}
					lines.push("</ul>");
					lines.push("</li>");
				}
				if ( !isEmpty(oClassInfo.aggregations) ) {
					...
					for (n in oClassInfo.aggregations) {
						if ( oClassInfo.aggregations[n].visibility !== "hidden" ) {
							lines.push("<li>{@link " + rname("get", n) + " " + n + "} : " +
								makeTypeString(oClassInfo.aggregations[n]) + (oClassInfo.defaultAggregation == n ? " (default)" : "") + "</li>");
						}
					}
					lines.push("</ul>");
					lines.push("</li>");
				...
				if ( !isEmpty(oClassInfo.associations) ) {
					...
					for (n in oClassInfo.associations) {
						lines.push("<li>{@link " + rname("get", n) + " " + n + "} : (sap.ui.core.ID | " +
							oClassInfo.associations[n].type + ")" + (oClassInfo.associations[n].cardinality === "0..n" ? "[]" : "") + "</li>");
					}
					lines.push("</ul>");
					lines.push("</li>");
				}
	...
	for (n in oClassInfo.events ) {
		...
		newJSDoc([
			"Attaches event handler <code>fnFunction</code> to the " + link + " event of this <code>" +
				oClassInfo.name + "</code>.",
			"",
		...
		newJSDoc([
			"Detaches event handler <code>fnFunction</code> from the " + link + " event of this <code>" +
				oClassInfo.name + "</code>.",
			"",
		...
		if ( info.allowPreventDefault ) {
			lines.push(
				...
				"Listeners may prevent the default action of this event by using the " +
					"<code>preventDefault</code>-method on the event object.",
				"");
		}
		lines.push(
			"",
...
exports.handlers = {
	...
	parseComplete: function(e) {
		...
		for (i = 0, j = 0; i < l; i++) {
			...
			if ( classInfos[doclet.longname] ) {
				...
				if ( doclet.__ui5.metadata.baseType
					...
					info("  @extends " + doclet.__ui5.metadata.baseType +
						" derived from UI5 APIs (" + doclet.longname + ")");
			...
			if ( j > 0 && doclets[j - 1].longname === doclet.longname ) {
				if ( !doclets[j - 1].synthetic && !doclet.__ui5.updatedDoclet ) {
					// replacing synthetic comments or updating comments are trivial case. Just log non-trivial
					// duplicates
					debug("ignoring duplicate doclet for " + doclet.longname + ":" + location(doclet) + " overrides " +
						location(doclets[j - 1]));
				}
				doclets[j - 1] = doclet;
			} else {
				doclets[j++] = doclet;
		...
		if ( pluginConfig.saveSymbols ) {
			...
			fs.writeFileSync(path.join(env.opts.destination, "symbols-parseComplete.json"),
				JSON.stringify(e.doclets, null, "\t"), "utf8");
		}
	}
};
...
exports.astNodeVisitor = {
	...
	visitNode: function(node, e, parser, currentSourceName) {
		...
		if (node.type === Syntax.ReturnStatement && node.argument &&
				node.argument.type === Syntax.ObjectExpression && /\.designtime\.js$/.test(currentSourceName) ) {
		...
		} else if ( node.type === Syntax.VariableDeclaration && node.declarations.length == 1 &&
			...
			processExtendCall(node.argument, comment, true);
				...
				isCreateDataTypeCall(node.expression.right) ) {
