...
function collectShortcuts(factory) {
	...
	} else if ( factory.type === Syntax.ArrowFunctionExpression && factory.expression === true) {
...
function collectDataTypeInfo(extendCall, classDoclet) {
	...
	const isArrowFuncExpression =
		def &&
		def.isValid &&
		def.isValid.value.type === Syntax.ArrowFunctionExpression &&
		def.isValid.value.expression === true;
	...
	if ( def
		 ...
		 && (isArrowFuncExpression || def.isValid.value.body.body.length === 1) ) {

		const varname = def.isValid.value.params[0].name;
		const stmt = isArrowFuncExpression ? def.isValid.value.body : def.isValid.value.body.body[0];

		if ( (stmt.type === Syntax.ReturnStatement && stmt.argument) || isArrowFuncExpression ) {
			const stmtArgument = isArrowFuncExpression ? stmt : stmt.argument;
			if ( stmtArgument.type === Syntax.CallExpression
				 && stmtArgument.callee.type === Syntax.MemberExpression
...
exports.astNodeVisitor = {
	...
	visitNode: function(node, e, parser, currentSourceName) {
		...
		const isArrowExpression = node.type === Syntax.ArrowFunctionExpression && node.expression === true && node.body.type === Syntax.ObjectExpression;
		if ((isArrowExpression || (node.type === Syntax.ReturnStatement && node.argument && node.argument.type === Syntax.ObjectExpression)) && /\.designtime\.js$/.test(currentSourceName) ) {
			const nodeArgument = isArrowExpression ? node.body : node.argument;
		...
		} else if ( (node.type === Syntax.ReturnStatement || (node.type === Syntax.ArrowFunctionExpression && node.expression === true)) 
					&& isExtendCall(node.argument || node.body) ) {

			const nodeArgument =
				node.type === Syntax.ArrowFunctionExpression && node.expression === true
					? node.body
					: node.argument;
