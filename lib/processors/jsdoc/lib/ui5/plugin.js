...
function collectShortcuts(factory) {
	...
	function checkAssignment(name, valueNode) {
		valueNode = resolvePotentialChainExpression(valueNode);
...
function resolvePotentialChainExpression(node) {
	let curNode = node;
	if ( node && node.type === "ChainExpression") {		
		curNode = node.expression;
	}
...
function isMemberExpression(node) {
	return node && [Syntax.MemberExpression, "OptionalMemberExpression"].includes(node.type);
}

function isCaleeMemberExpression(node) {
	return (
		node &&
		[Syntax.CallExpression, "OptionalCallExpression"].includes(node.type) &&
		isMemberExpression(node.callee)
	);
}

function isExtendCall(node) {
	node = resolvePotentialChainExpression(node);
...
function collectDataTypeInfo(extendCall, classDoclet) {
	...
	if ( i < args.length ) {
		const node = resolvePotentialChainExpression(args[i]);
	...
	if ( def
		...
		if ( (stmt.type === Syntax.ReturnStatement && stmt.argument) || isArrowExpression ) {
			const stmtArgument = resolvePotentialChainExpression(
	...
	visitNode: function(node, e, parser, currentSourceName) {
		...
		if ( node.type === Syntax.ExpressionStatement ) {
			if ( isSapUiDefineCall(node.expression) ) {
				analyzeModuleDefinition( resolvePotentialChainExpression(node.expression) );
			/*
		...
		} else if ( (node.type === Syntax.ReturnStatement || isArrowFuncExpression(node)) 
