...
function getEnclosingVariableScope (node) {
	...
	while (
		nearestScopeableNode.type !== Syntax.FunctionDeclaration &&
		nearestScopeableNode.type !== Syntax.FunctionExpression
	) {
		nearestScopeableNode = nearestScopeableNode.parent;
	}

	const scopeManager = escope.analyze(currentProgram);
	// Get the scope of the nearest scopeable node
	return scopeManager.acquire(nearestScopeableNode);
};
 ...
 * @returns 
 */
function getVarDestructuringChain (node){
	...
	const checkVarRenaming = (variable) => {
		...
		return variable.defs
			.filter(
				(def) =>
					def.node &&
					def.node.id &&
					def.node.id.type === Syntax.ObjectPattern
			) // Filter on object patterns => Detructure renamig
			.map((def) => def.node.id) // Drill down to the ObjectPattern Node
			.filter((objPatternNode) =>
				objPatternNode.properties.some(
					(prop) => prop.value.name === variable.name
				)
			) // Filter on ObjectPattern nodes that contain the variable
			.reduce((acc, objPatternNode) => {
				return acc.concat(
					objPatternNode.properties.filter(
						(prop) => prop.value.name === variable.name
					)
				);
			}, [])
	...
	// i.e. let {Foo} = myObject; let {Bar} = Foo; --> Bar
	while (leftMostName) {
		let potentialChunk = currentScope.variables
			.filter((curVar) => curVar.name === leftMostName)
			.reduce((acc, curVar) => {
				const potentialRenaming = checkVarRenaming(curVar);
				const filteredValues = curVar.references
					.filter((ref) => ref.writeExpr)
					.map((ref) => ref.writeExpr.name);
...
function convertValueWithRaw(node, type, propertyName) {
	...
	if ( node.type === Syntax.Literal ) {
		...
		const checkEnumsFromExternalModules = (varsDestructuringStack) => {
			const localName = varsDestructuringStack[0];
			const potentialEnumKey = varsDestructuringStack[varsDestructuringStack.length - 1];

			const localModule =
				currentModule.localNames[localName].module ||
				currentModule.localNames[localName].class;
			...
			return Object.keys(enumValues)
				.filter((curEnumKey) => {
					...
					if (enums.split("|").indexOf(potentialEnumKey) === -1) {
						return false;
					}
		...
		const resolvePotemtialEnum = (nodeToResolve) => {
			const varsDestructuringStack = getVarDestructuringChain(nodeToResolve);

			if (!varsDestructuringStack && !varsDestructuringStack.originalChain.length) {
				return getObjectName(nodeToResolve);
			}

			// TODO: After we've been through the chain up to the top of the module,
			// now we need to get the namespace from the external file if such exists.
			const fullyQualifiedLocalName = varsDestructuringStack.originalChain.join(".");
			const localName = varsDestructuringStack.originalChain[0];

			if (!currentModule.localNames[localName]) {
				return fullyQualifiedLocalName;
			}

			return checkEnumsFromExternalModules(varsDestructuringStack.originalChain);
		};

		const resolvedEnum = resolvePotemtialEnum(node);
