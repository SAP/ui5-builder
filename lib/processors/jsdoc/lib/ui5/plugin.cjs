	...
	if ( currentModule.dependencies && currentModule.factory ) {
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			let names;
			 ...
			 // ObjectPattern means destructuring of the parameter of a function
			if	(currentModule.factory.params[i].type === Syntax.ObjectPattern) {
				...
				names = currentModule.factory.params[i].properties.map(prop => {
					const resolvedVar = getEnclosingVariableScope(prop).set.get(prop.value.name);
					const renamedVar = checkVarRenaming(resolvedVar);

					return renamedVar || { original: prop.key.name };
				});
...
function resolvePotentialEnum(node, type) {
	const varsDestructuringStack = resolveFullyQuantifiedName(node, type);
...
function checkVarRenaming(variable) {
	...
	if (variable.defs && variable.defs.length !== 1) {
		return null;
	}
	...
	if (!defNode || defNode.type !== Syntax.ObjectPattern) { // Not a destructuring
            return null;
        }
...
function resolveFullyQuantifiedName(node) {
	...
	while (leftMostName) {
		...
		const defNode =
			getFuncArgumentDestructNode(curVar.defs[0], curVar.name) || // (arrow) function argument
			curVar.defs[0].node.id; // variable definition

		if (!defNode || defNode.type !== Syntax.ObjectPattern) {
			// Not a destructuring
