		...
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			...
			if	(currentModule.factory.params[i].type === Syntax.ObjectPattern) {
				...
				names = currentModule.factory.params[i].properties.map(prop => {
					...
					return { original: renamedVar.original, renamed: prop.value.name };
				});
...
function checkVarRenaming (variable) {
	...
	let potentialRename = variable.defs
		// Align structure from from variables comming from function parametters and ones defined in the current scope
		.reduce((acc, def) => {
			if (def.node && def.node.type === Syntax.ArrowFunctionExpression) {
				acc = acc.concat(def.node.params);
			} else if (def.node) {
				acc.push(def.node.id);
			}
			return acc;
		}, [])
		// Filter on object patterns => Detructure renamig
		.filter((param) => param && param.type === Syntax.ObjectPattern)
		// Drill down to the ObjectPattern Node
...
function resolveFullyQuantifiedName(node, type) {
	...
	while (leftMostName) {
		...
		const isReplace =
			potentialChunk &&
			potentialChunk.replaced &&
			potentialChunk.replaced !== potentialChunk.original;
