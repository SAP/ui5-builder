		...
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			...
			if	( [Syntax.ObjectPattern, Syntax.ArrayPattern].includes(currentModule.factory.params[i].type) ) { // ObjectPattern/ArrayPattern means destructuring of the parameter of a function
				names = resolveObjectPatternChain(currentModule.factory.params[i], null, [])
					.reduce((acc, {valueNode, keyNode, keyChain}) => {
						const isRename = keyNode.name !== valueNode.name;
						// keyChain > 1 means deep destructuring and we need to expose the outermost segment
						// No renaming is possible in such case, so we don't need to take care of this
						const isDeepDestructuring = keyChain.length > 1;

						let result = { original: keyNode.name, };
						if ( isRename ) {
							result.renamed = valueNode.name;
						}
						if ( isDeepDestructuring ) {
							acc.push({ original: keyChain[0] });
						}
						if (isRename || isDeepDestructuring) {
...
function resolveFullyQuantifiedName(node) {
	...
	const nameToResolve = renameMap[originalNameChunks[0]] || originalNameChunks[0];
	const localName = currentModule.localNames[originalNameChunks[0]];
	if (localName && !localName.path) {
		return [getResolvedName(nameToResolve, nameToResolve), originalName]
			.join(".")
			.split(".");
	} else {
		return getResolvedName(originalName, nameToResolve).split(".");
	}
}
