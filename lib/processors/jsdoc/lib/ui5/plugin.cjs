	...
	if ( currentModule.dependencies && currentModule.factory ) {
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			const names =
				(currentModule.factory.params[i].type === Syntax.ObjectPattern)
					? currentModule.factory.params[i].properties.map(prop => {
						...
						return renamedVar ? renamedVar.original : prop.value.name;
			...
			names.forEach(name => {
				...
				// TODO: localNames MUST have the renamed var as a key, but a property with the original name
				// For example: {"MyVarRenamed": {module: "/dependency1.js", property: "myVarOriginal"}}
				// In cases without a rename, MyVarRenamed === myVarOriginal
...
function checkVarRenaming (variable) {
	...
	let potentialRename = variable.defs
		.reduce((acc, def) => {
			if (def.node?.type === Syntax.ArrowFunctionExpression) {
		...
		.filter((param) => param.type === Syntax.ObjectPattern) // Filter on object patterns => Detructure renamig
		// .map(
		// 	(def) =>
	...
	while (leftMostName) {
		...
		if (potentialChunk && potentialChunk.renamed) {
			originalName = originalName.replace(
				new RegExp("(^|\.)" + potentialChunk.renamed + "($|\.)"),
				"$1" + potentialChunk.original + "$2"
			);
		}
	}
