...
function checkVarRenaming (variable) {
	...
	if (varDefinition.node.type === Syntax.ArrowFunctionExpression) {
		...
		defNode = varDefinition.node.params.find(
			(arg) =>
				arg.properties &&
				arg.properties.some((prop) => prop.value.name === variable.name)
		);
	} else { // Variable definition
		defNode = varDefinition.node.id;
	}
	...
	const varNode = defNode.properties.find( (prop) => prop.value.name === variable.name );


	if (varNode && varNode.key.name !== varNode.value.name) { // There's a rename
...
function resolveFullyQuantifiedName(node, type) {
	...
	while (leftMostName) {
		...
		let potentialChunk = {
			...
			original: curVar.name
		};

		if (potentialRenaming) {
			potentialChunk = Object.assign(potentialChunk, {
				original: potentialRenaming.original,
				renamed: potentialRenaming.renamed
			});
		}

		leftMostName = potentialChunk.ref;

		const isRename =
			potentialChunk &&
			potentialChunk.renamed &&
			potentialChunk.renamed !== potentialChunk.original;

		if ( isRename ) {
			const renamedChunks = originalName.split(".")
			renamedChunks.splice(0, 1, potentialChunk.original)
	...
	// TODO: Use currentModule.localNames to resolve coreLibrary -> sap.ui.core OR getResolvedObjectName()/getObjectName()  eventually?
	// getResolvedName(originalName);
