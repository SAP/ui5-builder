...
function checkVarRenaming(variable) {
	...
	const varNode = resolveObjectPatternChain(defNode, null, []).find(
		({valueNode}) => valueNode.name === variable.name
	);

	if (varNode && varNode.keyNode.name !== varNode.valueNode.name) { // There's a rename
		let result = { original: varNode.keyNode.name, renamed: varNode.valueNode.name };

		if (varNode.keyChain.length > 1 /*|| keyNode.name !== valueNode.name*/) {
			result.path = varNode.keyChain.join(".");
		}
