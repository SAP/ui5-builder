			...
			const names =
				(currentModule.factory.params[i].type === Syntax.ObjectPattern)
					? currentModule.factory.params[i].properties.map(prop => {
						const resolvedVar = getEnclosingVariableScope(prop)
							.variables.filter((curVar) => prop.value.name === curVar.name)[0];
...
function checkVarRenaming (variable) {
	...
	let potentialRename = variable.defs
		...
		.filter((param) => param && param.type === Syntax.ObjectPattern) // Filter on object patterns => Detructure renamig
		// .map(
		// 	(def) =>
		// 		def.node.id /* ObjectPattern */ ||
		// 		def.node.params /* ArrowFunctionExpression */
	...
	while (leftMostName) {
		...
		if (potentialChunk && potentialChunk.renamed) {
			originalName = originalName.replaceAll(
				...
				function (match, index, input) {
					var isBeginOrDotInFront = !index || input[index - 1] === ".";
					var charInTheEnd = input[index + match.length];
					var isEndOrDotInEnd = !charInTheEnd || charInTheEnd === ".";

					return isBeginOrDotInFront && isEndOrDotInEnd
 ...
 */
 function getEnclosingVariableScope (node) {
	...
	let neatestScope = scopeManager.acquire(nearestScopeableNode);
	while (
		!neatestScope &&
