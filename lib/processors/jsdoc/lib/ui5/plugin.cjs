...
function resolvePotemtialEnum(node) {
	const varsDestructuringStack = getVarDestructuringChain(node);
	...
	const fullyQualifiedLocalName =
		varsDestructuringStack.originalChain.join(".");
 ...
 */
 function getEnclosingVariableScope (node) {
	...
	while (
		nearestScopeableNode && nearestScopeableNode.type &&
		nearestScopeableNode.type !== Syntax.ArrowFunctionExpression &&
