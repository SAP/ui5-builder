			...
			if	( [Syntax.ObjectPattern, Syntax.ArrayPattern].includes(currentModule.factory.params[i].type) ) { // ObjectPattern/ArrayPattern means destructuring of the parameter of a function
				names = resolveObjectPatternChain(currentModule.factory.params[i], null, [])
					.reduce((acc, {valueNode, keyNode, keyChain}) => {
						...
						if ( keyNode.name !== valueNode.name ) { // Renaming
		...
		for (let i = 0; i < valueNode.elements.length; i++) {
			chainSequence = chainSequence.concat(
				resolveObjectPatternChain(
					...
					valueNode.elements[i],
					( valueNode.elements[i].name
						? [...keyChain, valueNode.elements[i].name]
						: [...keyChain, String(i)] )
				)
			);
		}
...
function checkVarRenaming(variable) {
	...
	if ( resolvedOP ) {
		let result = { original: resolvedOP.keyNode.name, path: resolvedOP.keyChain.join(".") };

		if (resolvedOP.keyNode.name !== resolvedOP.valueNode.name) {
			result.renamed = resolvedOP.valueNode.name;
		}
...
function resolveFullyQuantifiedName(node) {
	...
	const buildResult = (name, namespace) => {
		let result = getResolvedName(name, namespace);
		// Cleanup the merge from getResolvedName();
		// Strips the redundant parts of the name in the path + name
		if ( result.includes(name) && !result.endsWith(name) ) {
			result = result.replace(name.slice(name.indexOf(".")), "");
		}
	...
	while (leftMostName) {
		...
		if ( !isVarDestructuring(curVar) ) {
			// Not a destructuring
			return buildResult(originalName, leftMostName);
		}
		...
		if (potentialRenaming) {
			renameMap[potentialRenaming.path || potentialRenaming.original] =
				potentialRenaming.renamed /* local name */ || potentialRenaming.original /* no renaming */;
	...
	const nameToResolve =
		renameMap[originalNameChunks[0]] ||
		renameMap[originalName] ||
		originalNameChunks[0];

	return buildResult(originalName, nameToResolve);
}
