		...
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			...
			if	( [Syntax.ObjectPattern, Syntax.ArrayPattern].includes(currentModule.factory.params[i].type) ) { // ObjectPattern/ArrayPattern means destructuring of the parameter of a function
				names = resolveObjectPatternChain(currentModule.factory.params[i], null, [])
					.map(({valueNode, keyNode, keyChain}) => {
						let result = { original: keyNode.name, }
						if (keyNode.name !== valueNode.name) {
							result.renamed = valueNode.name;
						}
						if (keyChain.length > 1 || keyNode.name !== valueNode.name) {
							result.path = keyChain.join(".");
						}
...
function resolveObjectPatternChain (valueNode, keyNode, keyChain) {
	...
	if (valueNode && valueNode.type === Syntax.ObjectPattern) {
		...
		chainSequence.push({ valueNode, keyNode, keyChain });
	}
...
function checkVarRenaming(variable) {
	...
	const resolvedOP = resolveObjectPatternChain(defNode, null, []).find(
		({valueNode}) => valueNode.name === variable.name
	);

	if (resolvedOP && resolvedOP.keyNode.name !== resolvedOP.valueNode.name) { // There's a rename
		let result = { original: resolvedOP.keyNode.name, renamed: resolvedOP.valueNode.name };

		if (resolvedOP.keyChain.length > 1 || resolvedOP.keyNode.name !== resolvedOP.valueNode.name) {
			result.path = resolvedOP.keyChain.join(".");
		}
...
function resolveFullyQuantifiedName(node) {
	...
	while (leftMostName) {
		...
		if (potentialRenaming) {
			...
			renamedChunks.splice(0, 1, potentialRenaming.path || potentialRenaming.original);
