...
function analyzeModuleDefinition(node) {
	...
	if ( currentModule.dependencies && currentModule.factory ) {
		for ( let i = 0; i < currentModule.dependencies.length && i < currentModule.factory.params.length; i++ ) {
			...
			if (currentModule.factory.params[i].type === Syntax.ArrayPattern) { // ArrayPattern means destructuring of the parameter of a function
				names = currentModule.factory.params[i].elements.map((param) => {
					return {original: param.name};
				});
			} else if	(currentModule.factory.params[i].type === Syntax.ObjectPattern) { // ObjectPattern means destructuring of the parameter of a function
				// There might be multiple variables defined in the destructor and even further-
				// those variables to be renamed i.e. function doSomething({a, b: renamedB}, c) {
				names = currentModule.factory.params[i].properties.reduce((acc, prop) => {

					// Resolves nested destructuring with (potential) multiple end nodes
					const resolveObjectPatternChain = (prop, keys) => {
						let chainSequence = [];
						if (prop.value && prop.value.type === Syntax.ObjectPattern) {
							for (let i = 0; i < prop.value.properties.length; i++) {
								chainSequence = chainSequence.concat(
									resolveObjectPatternChain( prop.value.properties[i], [...keys, prop.key.name] )
								);
							}
						} else {
							chainSequence.push({ prop, keys });
						}

						return chainSequence;
					};

					const objectPatternIdentifiers = resolveObjectPatternChain(prop, []);

					objectPatternIdentifiers.forEach(({prop, keys}) => {
						if (prop.key.name !== prop.value.name) {
							acc.push({ original: prop.key.name, renamed: prop.value.name, path: keys.join(".") });
						} else {
							acc.push({ original: prop.key.name, path: keys.join(".") });
						}
					});
...
function resolvePotentialEnum(node, type) {
	const varsDestructuringStack = resolveFullyQuantifiedName(node);
 ...
 * @returns {Node}
 */
function getFuncArgumentDestructNode(varDefinition, varName) {
	if (varDefinition.node.type === Syntax.ArrowFunctionExpression) {
		// If destructuring happens as an ArrowFunction argument, then we need to find the exact argument.
		// The structure is as follows:
		// variable.defs.node.params[].properties[], where
		// - node: ArrowFunctionExpression
		// - node.params: ArrowFunctionExpression arguments
		// - node.params[].properties: Destructured variables (Nodes). Represents 'b' and 'c' in: (a, {b, c}) => {}
		return varDefinition.node.params.find(
			(arg) =>
				arg.properties &&
				arg.properties.some((prop) => prop.value.name === varName)
		);
	}
...
function isVarDestructuring(variable) {
	const defNode =
		variable &&
		variable.defs &&
		( getFuncArgumentDestructNode(variable.defs[0], variable.name) // (arrow) function argument
		|| variable.defs[0].node.id ); // variable definition

	return defNode && defNode.type === Syntax.ObjectPattern;
}
...
function checkVarRenaming(variable) {
	...
	if (
		!variable || !variable.defs
		|| (variable.defs && variable.defs.length !== 1)
		|| !isVarDestructuring(variable)
	) {
		return null;
	}

	const varDefinition = variable.defs[0];
	const defNode = getFuncArgumentDestructNode(varDefinition, variable.name) // (arrow) function argument
		|| varDefinition.node.id; // variable definition

	const varNode = defNode.properties.find(
		(prop) => prop.value.name === variable.name
	);

	if (varNode && varNode.key.name !== varNode.value.name) {
		// There's a rename
		return { original: varNode.key.name, renamed: varNode.value.name };
	} else {
		return null;
	}
};
...
function resolveFullyQuantifiedName(node) {
	...
	while (leftMostName) {
		...
		if ( !isVarDestructuring(curVar) ) {
			// Not a destructuring
			return getResolvedName(originalName, leftMostName).split(".");
		}
		...
		if (potentialRenaming && potentialRenaming.original !== potentialRenaming.renamed) {
			renameMap[potentialRenaming.original] = potentialRenaming.renamed

			const renamedChunks = originalName.split(".")
			renamedChunks.splice(0, 1, potentialRenaming.original)
			originalName = renamedChunks.join(".");
		}

		// This is a destructured function argument.
		// We'd need to determine the external module (getResolvedName() -> currentModule.localNames[])
		// and append it to the argument's name.
		if ( !!getFuncArgumentDestructNode(curVar.defs[0], curVar.name) ) {
			const nameToResolve = renameMap[curVar.name] || curVar.name;
			originalName =
				[getResolvedName(nameToResolve, nameToResolve), originalName].join(".");
		} else if (leftMostName) {
			originalName = leftMostName + "." + originalName;
		}
	}

	const originalNameChunks = originalName.split(".");
	const resolvedNamespace = getResolvedName(originalNameChunks[0], originalNameChunks[0]).split(".");

	return resolvedNamespace.concat(originalNameChunks.slice(1));
}
 ...
 */
 function getEnclosingVariableScope (node) {
	...
	// Get the scope of the nearest scopeable node
	return nearestScope;
};
...
function convertValueWithRaw(node, type, propertyName) {
	...
	if ( node.type === Syntax.Literal ) {
		...
		// This could be an ENUM which has been destructed up to the entity part. In that case the node.type === Syntax.Identifier
