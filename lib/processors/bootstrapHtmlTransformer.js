 ...
 * @param {function} [parameters.options.rewriteSrc] todo
...
module.exports = async function({resources, options}) {
	if (options.src && options.rewriteSrc) {
		throw new Error("Parameters src and rewriteSrc can't be used together");
	}
	async function processResource(resource) {
		...
		if (bootstrapScript.length === 1) {
			...
			if (options.src) {
				newSrc = options.src;
			} else if (options.rewriteSrc) {
				newSrc = await options.rewriteSrc(bootstrapScript.attr("src"));
				bootstrapScript.attr("data-sap-ui-main", "sap/xx/proxy/boot");
			}
			bootstrapScript.attr("src", newSrc);
