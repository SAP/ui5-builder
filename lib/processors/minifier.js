...
export default async function({resources, taskUtil, options: {addSourceMappingUrl = true, useWorkers = false} = {}}) {
	...
	return Promise.all(resources.map(async (resource) => {
		...
		const result = await minify({
			filename,
			dbgFilename,
			code: new Int8Array(code.buffer, code.byteOffset, code.length),
			sourceMapOptions
		}, taskUtil);
		resource.setBuffer(Buffer.from(result.code.buffer, result.code.byteOffset, result.code.length));
		const sourceMapResource = new Resource({
			path: resource.getPath() + ".map",
			buffer: Buffer.from(result.map.buffer, result.map.byteOffset, result.map.length)
		});
