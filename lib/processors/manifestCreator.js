...
async function createManifest(libraryResource, libBundle, descriptorVersion, _include3rdParty) {
	...
	async function createSapApp() {
		async function isEmbeddedByLibrary(componentPath, libraryPathPrefix) {
			...
			let resolvedEmbeddedBy = resolvePath(embeddedBy, componentPath);
		...
		async function findEmbeddedComponents() {
			...
			for (const comp of components) {
				...
				const relativePath = componentPath.slice(prefix.length);
				if ( relativePath.lastIndexOf("/") >= 0 ) {
					if (await isEmbeddedByLibrary(componentPath, prefix)) {
		...
		function resolvePath(relativePath, basePath) {
			...
			return posixPath.resolve(basePath, relativePath);
		}
