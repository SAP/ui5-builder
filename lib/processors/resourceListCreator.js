...
const LocatorResourcePool = require("../lbt/LocatorResourcePool");
...
class ResourceInfo {
	...
	toJSON() {
		...
		if ( this.exposedGlobalNames != null && this.exposedGlobalNames.size > 0 ) {
			result.exposedGlobals = [...this.exposedGlobalNames];
		}
		if ( this.format ) {
...
class ResourceCollector {
	...
	async enrichWithDependencyInfo(resourceInfo) {
		return this._pool.getModuleInfo(resourceInfo.name).then((info) => {
			...
			if (info.requiresTopLevelScope) {
				resourceInfo.requiresTopLevelScope = true;
			}
			if (info.exposedGlobals != null && info.exposedGlobals.length) {
				resourceInfo.exposedGlobalNames = resourceInfo.exposedGlobalNames || new Set();
				info.exposedGlobals.forEach((exposedGlobalName) => {
	...
	createOrphanFilters() {
		log.verbose(
			"  configured external resources filters (resources outside the namespace): %s",
			this._externalResources == null ? "(none)" : this._externalResources);

		const filters = new Map();

		if ( this._externalResources != null ) {
			for ( let component in this._externalResources ) {
				const packageFilters = new ResourceFilterList(this._externalResources[component]);
				...
				filters.set(component, packageFilters);
			}
		}
		return filters;
	}
...
const getResourceSize = async (resource) => {
	try {
		// this retrieves the byteSize of the resource
	...
	if (resource.getStatInfo().size !== undefined) {
		return resource.getStatInfo().size;
	}
	return -1;
};
...
module.exports = async function({resources, dependencyResources}, options) {
	...
	const visitorsPromise = resources.map((resource) => {
		return getResourceSize(resource).then((size) => {
			return collector.visitResource(resource.getPath(), size);
		});
	});
	await Promise.all(visitorsPromise);
