...
class ResourceCollector {
	...
	async determineResourceDetails({pool, debugResources, mergedResources, designtimeResources, supportResources}) {
		...
		for (const [name, info] of this._resources.entries()) {
			...
			// set the module name for .properties
...
module.exports = async function({resources}, options) {
	...
	resources.forEach((resource) => {
		collector.visitResource(resource.getPath(), resource.getStatInfo().size);
	});
	...
	for (const [prefix, list] of collector.components.entries()) {
		...
		const prevContentString = JSON.stringify(list, null, "\t");
		const resourcesJson = new ResourceInfo(prefix + "resources.json");
		resourcesJson.size = Buffer.from(prevContentString).byteLength;
		const stringEntryToAdd = JSON.stringify(resourcesJson, null, "\t");
		resourcesJson.size = Buffer.from(prevContentString + stringEntryToAdd).byteLength;
		list.add(resourcesJson);

		const contentString = JSON.stringify(list, null, "\t");
