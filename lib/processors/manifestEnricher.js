...
function resolveUI5Url(sUrl) {
	...
	if (sUrl.startsWith("ui5:")) {
		...
		if (!sNoScheme.startsWith("//")) {
			// TODO: Also throw an error here? Rather ignore the entry and log warning/verbose?
			throw new Error("URLs using the 'ui5' protocol must be absolute. " +
...
class ManifestEnricher {
	...
	getType() {
		return this.manifest["sap.app"].type;
	}
	...
	async processBundleConfig(bundleConfig) {
		...
		const bundleUrl = getBundleUrlFromConfig(bundleConfig, sapAppId);
		if (!bundleUrl) {
			return;
		}
		...
		const normalizedBundleUrl = normalizeBundleUrl(bundleUrl, sapAppId);
		if (normalizedBundleUrl.startsWith("../")) {
			log.verbose(
				`Resolved bundleUrl '${bundleUrl}' points to a bundle outside of the ` +
	...
	async processSapUi5ModelConfig(modelConfig) {
	...
	async processSapUi5Models() {
		...
		await Promise.all(
			modelConfigs.map((modelConfig) => this.processSapUi5ModelConfig(modelConfig))
		);
	}
	...
	async processSapUi5LibraryI18n() {
		...
		if (sapUi5LibraryI18n?.terminologies) {
			bundleConfigs.push(...Object.values(sapUi5LibraryI18n.terminologies));
		}

		sapUi5LibraryI18n?.enhanceWith?.forEach((config) => {
			bundleConfigs.push(config);
			if (config.terminologies) {
				bundleConfigs.push(...Object.values(config.terminologies));
			}
	...
	async run() {
		...
		const descriptorVersion = new Version(this.manifest._version);
