...
class ManifestEnricher {
	...
	async processBundleConfig(bundleConfig) {
		const bundleUrl = getBundleUrlFromConfig(bundleConfig, this.manifest["sap.app"].id);
		if (!bundleUrl) {
			return;
		}
		...
		const normalizedBundleUrl = normalizeBundleUrl(bundleUrl, this.manifest["sap.app"].id);
