...
class ManifestEnricher {
	...
	async processBundleConfig(bundleConfig, fallbackBundleUrl, isTerminologyBundle = false) {
	...
	async processTerminologiesAndEnhanceWith(bundleConfig) {
		...
		await Promise.all(
			terminologyBundleConfigs.map((bundleConfig) => this.processBundleConfig(bundleConfig, undefined, true))
		);
	}
	...
	async processSapUi5Models() {
		...
		await Promise.all(
			modelConfigs.map(async (modelConfig) => {
				...
				await this.processBundleConfig(settings, modelConfig.uri);
