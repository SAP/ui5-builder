...
class ThemeBuilder {
	 ...
	 * @param {boolean} [options.cssVars=true] Generates the CSS variables (css-variables.css, css-variables.source.less) and the skeleton for a theme (library-skeleton.css, [library-skeleton-RTL.css])
	 * @returns {Promise<module:@ui5/fs.Resource[]>} Resolving with array of created files
	 */
	build(resources, {compress = false, cssVars = true} = {}) {
		const files = [];
			...
			return this.builder.build({
				...
				if (result.cssVars) {
					const libCssVarsSource = new Resource({
						...
						string: result.cssVars.lessVariables
					});
					const libCssVars = new Resource({
						path: themeDir + "/css-variables.css",
						string: result.cssVars.cssVariables
					});
