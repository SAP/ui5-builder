...
class ThemeBuilder {
	 ...
	 * @param {boolean} [options.cssVars=false] Generates the CSS variables (css-variables.css, css-variables.source.less) and the skeleton for a theme (library-skeleton.css, [library-skeleton-RTL.css])
	 * @returns {Promise<module:@ui5/fs.Resource[]>} Resolving with array of created files
	 */
	build(resources, {compress = false, cssVars = false} = {}) {
		const files = [];

		// TODO: remove the next two lines when ready!
		compress = false;
		cssVars = true;
