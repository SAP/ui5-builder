...
class ThemeBuilder {
	 ...
	 * @param {boolean} [options.rtl=true] Generate the RTL variant of the theme
	 * @returns {Promise<module:@ui5/fs.Resource[]>} Resolving with array of created files
	 */
	build(resources, {compress = false, cssVariables = false, rtl = true} = {}) {
		const files = [];
		...
		const compile = (resource) => {
			...
			return this.builder.build({
				...
				if (cssVariables) {
					...
					files.push(libCssVarsSource, libCssVars, libCssSkel);
