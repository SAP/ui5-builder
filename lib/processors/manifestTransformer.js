...
function isAutofillRequired(bundleConfig, appId) {
	...
	if (bundleUrl && bundleUrl.startsWith("..")) {
		// TODO: check is not correct, if the manifest.json is part of a reuse component and consumes the library bundle
		// the bundleUrl might start with a ".."
		// Option 1: resolve bundeUrl and check if it matches the given namespace task option
		// Option 2: use fs.stat on the ui5 project to check if bundle is there
...
async function transformManifest(resource, readdir, options) {
	...
	await Promise.all(bundles.map(async (bundleConfig) => {
		...
		if (bundleUrl) {
			...
			if (bundleUrl.startsWith("ui5://")) {
				defaultBundleName = defaultBundleName.replace("ui5://", "").replace(namespace, "");
			}
		}
		...
		if (isNamespaced) {
			pathToDefaultBundleName = path.join(`/resources/${namespace}`, defaultBundleName);
		} else {
