...
export class FsMainThreadInterface {
	...
	constructor(port, fsInterfaceReader) {
		if (!port) {
		...
		this.#port.on("close", this.endCommunication.bind(this));
	}

	#onMessage(e) {
		switch (e.what) {
		case "readFile":
			this.#doRequest({what: "readFile", fsPath: e.fsPath, options: e.options});
			break;
		case "stat":
			this.#doRequest({what: "stat", fsPath: e.fsPath});
			break;
		case "readdir":
			this.#doRequest({what: "readdir", fsPath: e.fsPath});
			break;
		case "mkdir":
			this.#doRequest({what: "mkdir", fsPath: e.fsPath});
			break;
		}
	}

	#doRequest({what, fsPath, options}) {
		const cacheKey = [fsPath, what].join("-");
		const cb = (error, result) => {
			if (!error && !this.#cache[cacheKey]) {
				this.#cache[cacheKey] = result;
			}
			this.#port.postMessage({what, error, result, fsPath});
		};
...
export class FsWorkerThreadInterface {
	...
	#doRequest({what, fsPath, options}, callback) {
		...
		if (this.#cache[cacheKey]) {
			...
			this.#port.postMessage({what, fsPath, options});
		}
	}
