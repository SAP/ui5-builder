		...
		if (!this.#cache[cacheKey]) {
			this.#cache[cacheKey] = new Promise((res) => {
				if (what === "readFile") {
					...
					this.#fsInterfaceReader[what](fsPath, (error, result) => res({error, result}));
				}
			});
		}

		const fromCache = await this.#cache[cacheKey];
		comPort.postMessage({what, fsPath, ...fromCache});
	}
}
...
export class FsWorkerThreadInterface {
	...
	#onMessage(e) {
		...
		if (cbObject) {
			this.#cache[[e.fsPath, e.what].join("-")] = {error: e.error, result: e.result};
