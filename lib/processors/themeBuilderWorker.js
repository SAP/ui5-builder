...
export class FsMainThreadInterface {
	...
	#onMessage(e, comPort) {
		...
		case "stat":
			...
			break;
		case "readdir":
			this.#doRequest(comPort, {action: "readdir", fsPath: e.fsPath});
			break;
		case "mkdir":
			this.#doRequest(comPort, {action: "mkdir", fsPath: e.fsPath});
			break;
		}
	}
		...
		if (!this.#cache[cacheKey]) {
			this.#cache[cacheKey] = new Promise((res) => {
				if (action === "readFile") {
					...
					this.#fsInterfaceReader[action](fsPath, (error, result) => res({error, result}));
				}
			});
		}
...
export class FsWorkerThreadInterface {
	...
	#onMessage(e) {
		...
		if (cbObject) {
			this.#cache[[e.fsPath, e.action].join("-")] = {error: e.error, result: e.result};
	...
	readdir(fsPath, callback) {
		this.#doRequest({action: "readdir", fsPath}, callback);
	}

	mkdir(fsPath, callback) {
		this.#doRequest({action: "mkdir", fsPath}, callback);
	}
}
