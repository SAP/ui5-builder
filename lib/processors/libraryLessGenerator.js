...
class LibraryLessGenerator {
	...
	async generate({filePath, fileContent}) {
		let content =
		`/* NOTE: This file was generated as an optimized version of ` +
		...
		const isBaseTheme = posixPath.dirname(filePath).endsWith("/themes/base");
		if (!isBaseTheme) {
			// CSS Variables should only be declared in the actual theme.
			// Otherwise there would be multiple :root declarations
			content += `\n\n@import "css_variables_only.less";`;
		}
		return content;
	}
