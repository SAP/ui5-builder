...
test("getManifest: reads correctly", async (t) => {
	...
	const res = await libraryFormatter.getManifest();
	t.deepEqual(res._version, "1.1.0", "manifest.json content has been read");
});
...
test.serial("getManifest: invalid JSON", async (t) => {
	...
	t.deepEqual(error.message,
		"Failed to read manifest.json for project random.project: " +
		"Unexpected token p in JSON at position 0",
		"Rejected with correct error message");
	t.deepEqual(readFileStub.callCount, 1, "fs.read got called once");
	const expectedPath = path.join(applicationBPath, "pony-path", "manifest.json");
...
test.serial("getManifest: result is cached", async (t) => {
	...
	let res = await libraryFormatter.getManifest();
	t.deepEqual(res, {pony: "no unicorn"}, "Correct result on first call");
	res = await libraryFormatter.getManifest();
