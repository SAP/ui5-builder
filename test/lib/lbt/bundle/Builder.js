...
test("integration: ES2023 Support with usePredefineCalls=true", async (t) => {
	const pool = new ResourcePool();
	pool.addResource({
		name: "myModule.js",
		getPath: () => "myModule.js",
...
const bla = {};`
	...
	const builder = new Builder(pool);
	const oResult = await builder.createBundle(bundleDefinition, {numberOfParts: 1, decorateBootstrapModule: true, usePredefineCalls: true});
	t.is(oResult.name, "Component-preload.js");
	const expectedContent = `//@ui5-bundle Component-preload.js
